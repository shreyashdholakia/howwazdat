    ...
    Team.findOne({'teamName': teams[team].teamName}, function (err, teamDetails) {
      ...
      if (teamDetails) {
        ...
        for(playerInfo in players) {
          playerStats.push({
            player: players[playerInfo].firstName + " " + players[playerInfo].lastName,
          ...
          Statistics.find({$and: [{'tournament': tournament}, {'player.team': teams[team].teamName}]}, function (err, tournamentStats) {
            if (err) {
              return next(new Error('Failed to load statistics ' + tournamentStats));
            }
            console.log(tournamentStats.length);
