 ...
 */
 ...
 exports.addMatches = function(req, res) {
 	...
 	Tournament.findOne({ 'tournamentName': tournament }, function (err, tournamentDetails) {
 		...
 		if(tournamentDetails) {
 			tournamentDetails.matches = req.body.matches;
 			tournamentDetails.save(function (err, done) {
 				if (!err) {
 					res.json({status: 'OK', data: tournamentDetails});
 				} else {
 	...
 	for(var i = 0; i < req.body.matches.length; i++) {
	 	var newMatch = new Matches ({
	 		...
	 		homeTeam:req.body.matches[i].homeTeam,
	 		visitingTeam:req.body.matches[i].visitingTeam,
	 		matchDate:req.body.matches[i].matchDate,
	 		matchNumber: req.body.matches[i].matchNumber
	 	});
