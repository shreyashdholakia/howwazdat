  ...
  Matches.findOne({'matchNumber': match}, function (err, matchDetails) {
    ...
    if(matchDetails) {
      if(battingTeam === 'home') {
        ...
        matchDetails.visitingTeamBatting.forEach(function (batter) {
          if(batter.email === scores.new.email) {
            runs = scores.new.runs - batter.runs;
            console.log(runs);
            balls = scores.new.balls - batter.balls;
...
exports.updateBowlingScore = function (req, res) {
  var team = req.params.team;
  var scores = req.body.scores[0];
  var tournament = req.body.scores[0].tournament;
  var match = req.body.scores[0].matchNumber;
  var bowlingTeam = req.body.scores[0].bowlingTeam;
  var runs = 0, overs = 0, wickets = 0, fiveFors = 0;
  var econ = (scores.new.runs / scores.new.overs);
  ...
  Matches.findOne({'matchNumber': match}, function (err, matchDetails) {
    ...
    if(matchDetails) {
      if(bowlingTeam === 'home') {
        matchDetails.homeTeamBowling.forEach(function (batter) {
          if(batter.email === scores.new.email) {
            runs = scores.new.runs - batter.runs;
            overs = scores.new.overs - batter.overs;
            wickets = scores.new.wickets - batter.wickets;
            if(batter.wickets >= 5 && scores.new.wickets < 5) {
              fiveFors = -1;
            }
            if(batter.wickets  < 5 && scores.new.wickets >= 5) {
              fiveFors = 1;
        ...
        matchDetails.visitingTeamBowling.forEach(function (batter) {
          if(batter.email === scores.new.email) {
            runs = scores.new.runs - batter.runs;
            overs = scores.new.overs - batter.overs;
            wickets = scores.new.wickets - batter.wickets;
      ...
      Statistics.findOneAndUpdate({$and: [{'tournament': tournament}, {'teams.email': scores.new.email}]}, {
        $inc: {
          'teams.$.runsGiven': runs,
          'teams.$.overs' : overs,
          'teams.$.wickets' : wickets,
          'teams.$.fiveFors' : fiveFors
        }
      }, {upsert: true}, function (err, stats) {
        if (err) return console.log(err);
        else res.json({status: 'OK', data: stats});
      ...
      Team.findOneAndUpdate({$and: [{'teamName': team}, {'players.email': scores.new.email}]}, {
        $inc: {
          'players.$.runsGiven': runs,
          'players.$.ballBowled': overs,
          'players.$.wickets' : wickets,
          'players.$.fiveFors' : fiveFors
        }
      }, {upsert: true}, function (err, teamInfo) {
        if (err) return console.log(err);
        else res.json({status: 'OK', data: teamInfo});
      ...
      if(bowlingTeam === 'home') {
        Matches.findOneAndUpdate({$and: [{'matchNumber': match}, {'homeTeamBowling.email': scores.new.email}]}, {
          $set: {
            'homeTeamBowling.$.runs': scores.new.runs,
            'homeTeamBowling.$.overs' : scores.new.overs,
            'homeTeamBowling.$.maiden' : scores.new.maiden,
            'homeTeamBowling.$.wickets' : scores.new.wickets,
            'homeTeamBowling.$.econ' : econ
          }
        }, {upsert: true}, function (err, matchInfo) {
          if (err) return console.log(err);
          else res.json({status: 'OK', data: matchInfo});
        ...
        Matches.findOneAndUpdate({$and: [{'matchNumber': match}, {'visitingTeamBowling.email': scores.new.email}]}, {
          $set: {
            'visitingTeamBowling.$.runs': scores.new.runs,
            'visitingTeamBowling.$.overs' : scores.new.overs,
            'visitingTeamBowling.$.maiden' : scores.new.maiden,
            'visitingTeamBowling.$.wickets' : scores.new.wickets,
            'visitingTeamBowling.$.econ' : econ
          }
        }, {upsert: true}, function (err, matchInfo) {
          if (err) return console.log(err);
          else res.json({status: 'OK', data: matchInfo});
  ...
  if(battingDetails && battingDetails.length > 0) {
    ...
    Team.findOneAndUpdate({$and: [{'teamName': team}, {'players.email': details.email}]}, {
          $inc: {
            ...
            'players.$.ballBowled': details.overs,
            'players.$.wickets': details.wickets,
            'players.$.fiveFors': fiveWickets
          }
        }, {upsert: true}, function (err, doc) {
          if (err) return console.log(err);
        });
