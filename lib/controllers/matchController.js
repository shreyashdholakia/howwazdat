    ...
    Points.findOne({'teamStats.team': match.winningTeam}, function (err, points) {
      ...
      points.teamStats.forEach(function (teams) {
        if (teams.team === match.winningTeam) {
            ...
            console.log("score" + points.teamStats.lost + "won" +  wonWinning + "points" + pointWinning);
        } else if (teams.team === match.losingTeam) {
            lostLosing = (Number(points.teamStats.lost) + 1);
            wonLosing = (Number(points.teamStats.won) - 1);
            pointLosing = (Number(points.teamStats.points) - 2);
          console.log("score" + wonLosing + "won" +  lostLosing + "points" + pointLosing);

        }
      });

      Points.findOneAndUpdate({'teamStats.team': match.winningTeam}, {$inc:  {'teamStats.$.points': 2, 'teamStats.$.won': 1}}, {$dec: {'teamStats.$.lost': 1}}, {upsert:true}, function(err, doc){
