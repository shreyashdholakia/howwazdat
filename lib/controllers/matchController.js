...
exports.updateMatchScores = function (req, res) {
  ...
  Matches.findOne({'matchNumber': match.matchNumber}, function (err, matchDetails) {
    ...
    if (matchDetails) {
      ...
      updatePlayerScores(tournament, match.homeTeam, match.homeTeamBatting);
      matchDetails.updatedBy = match.updatedBy;
      matchDetails.lastUpdated = match.lastUpdated;
...
function updatePlayerScores(tournament, team, battingDetails) {
  if(battingDetails && battingDetails.length > 0) {
    ...
    battingDetails[0].battingScores.forEach(function (scores) {
      console.log(scores);
      Statistics.findOneAndUpdate({$and: [{'tournament': tournament}, {'teams.team': team}, {'teams.player': scores.player}]}, {
          $inc: {
            'teams.$.matches': 1,
            'teams.$.runs': scores.runs,
            'teams.$.inning': 1
          }
        }, {upsert: true}, function (err, doc) {
