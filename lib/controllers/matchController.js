...
exports.update = function(req, res) {
  ...
  if(match.scoreCard == 'changed') {
    ...
    Points.update({'teamStats.team': 'Sixers'}, {'$set':  {'teamStats.$.points': '15'}});
    ...
    Points.findOne({'teamStats.team': match.winningTeam}, function (err, points) {
      ...
      points.teamStats.forEach(function (teams) {
        if (teams.team === match.winningTeam) {
          teams.points = (Number(teams.points) + 6);
        }
      });

      console.log(points);

      points.save(function(err, point){
        if(err) return console.error(err);
        else res.json({status: 'OK'});
      });


      //for(i in points.teamStats) {
      //  if(points.teamStats[i].team === match.winningTeam) {
      //    lostWinning = (Number(points.teamStats[i].lost) - 1);
      //    wonWinning = (Number(points.teamStats.won) + 1);
      //    pointWinning = (Number(points.teamStats[i].points) + 6);
      //    points.teamStats[i].points = '15';
      //    points.save(function(err, point){
      //      if(err) return console.error(err);
      //      else res.json({status: 'OK'});
      //    });
    ...
    Points.findOne({'teamStats.team': match.winningTeam}, function (err, points) {
      var index = points.teamStats.indexOf(match.winningTeam) > -1;
      console.log(index);
    });
